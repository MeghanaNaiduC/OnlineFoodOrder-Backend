...
DsJwtAuth.getToken = async function _getToken() {
  ...
  dsApi.setOAuthBasePath(dsConfig[param+'authServer']);
  console.log('vijay',dsApi)
  const results = await dsApi.requestJWTUserToken(dsConfig[param+'DocCID'],
          dsConfig[param+'impersonatedUserGuid'], scopes, dsConfig[param+'privateKey'],
          jwtLifeSec);
          console.log('kumar',results)
...
DsJwtAuth.getUserInfo = async function _getUserInfo(){
  ...
  const dsApi = new docusign.ApiClient()
      , targetAccountId = false //dsConfig.targetAccountId
